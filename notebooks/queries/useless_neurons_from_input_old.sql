...
useless_neurons_overview AS (
    SELECT
        *,
        ABS(eval - eval_prime) AS diff
    FROM
    (
        SELECT
            h.id AS removed_neuron_id,
            t_out.value AS eval,
            (
                WITH tx AS (
                    SELECT
                    ...
                    JOIN node n ON e.dst = n.id
                    -- Filter out the hidden node
                    WHERE e.dst <> h.id AND e.src <> h.id
                    GROUP BY e.dst, n.bias, tx.input_set_id
                ),
                t_out AS (
                    SELECT
                ...
                SELECT t_out.value FROM t_out
            ) AS eval_prime
        FROM t_out
        CROSS JOIN hidden_nodes h
    )
)
SELECT * FROM useless_neurons_overview;
